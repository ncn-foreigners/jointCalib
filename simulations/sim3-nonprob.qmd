---
title: "Simulation 3 - non-probability sample (known population totals)"
format: 
  html:
    self-contained: true
...
results_summary <- results_df[, .(m = mean(value), var = var(value)), keyby=.(data, y, type, estimator)]
results_summary[pop_true_vals, true := i.true, on = c("y", "type")]
results_summary[, bias:=m-true]
results_summary[, mse:=bias^2+var]
results_summary[, rmse:=sqrt(mse)]
results_summary[estimator%in% c("cal", "qcal1", "qcal2", "naive") & type == "q50" & data == "bd2", 
                .(data, y,type, estimator, bias, rmse, bias_rel =bias/true*100)]
```
```{r}
results_df[type == "mean"][, estimator:=factor(estimator, c("naive", "cal", "qcal1", "qcal2"))] |> 
  ggplot(data=_,aes(x = estimator, y= value)) +
  geom_hline(data=pop_true_vals[type == "mean"], aes(yintercept = true), linetype="dashed") + 
  geom_boxplot() + facet_grid(y~data, 
                              scales = "free_y") -> p1
...
```{r}
results_df[type == "q50"][, estimator:=factor(estimator, c("naive", "cal", "qcal1", "qcal2"))] |> 
  ggplot(data=_,aes(x = estimator, y= value)) +
  geom_hline(data=pop_true_vals[type == "q50" & y %in% c("y11", "y12")], 
...
ggsave(plot =p1, filename = "../figs/sim-nonprob-cases-q50.pdf", width = 6, height = 8)
```