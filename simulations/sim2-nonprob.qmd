...
results |> 
  ...
  geom_hline(data = pop_true_vals[, .(y, estimator=estimator2, true)], 
             aes(yintercept = true), linetype = "dashed", color = "red") + 
  labs(x = "Estimator", y = "Value") -> p1

p1
ggsave(plot = p1, filename = "../figs/sim-nonprob-known.pdf", width = 8, height = 6)
```

```{r}
results[, .(mean = mean(value), bias=mean(value)-mean(true), var = var(value)), .(y, estimator, type)][
  , ":="(mse = bias^2+var, rmse = sqrt(bias^2+var))][order(y, estimator)][
    , .(y, 
        characteristic=estimator,
        estimator=factor(type, c("naive", "cal", "qcal1", "qcal2"), c("Naive", "Cal", "QCal1", "QCal2")), 
        bias, var, rmse)] |> 
  ...
  {\(x) x[, .(characteristic, estimator, 
              y1_bias, y1_var, y1_rmse, y2_bias, y2_var, y2_rmse, 
              y3_bias, y3_var, y3_rmse)]}() -> results_sim_known
results_sim_known
```
...
```{r}
results_y3_sample <- results_y2_sample <- results_y1_sample <- matrix(data=0, nrow = n_reps, ncol = (1+length(p_quantiles))*4+1)
```
...
for (r in 1:n_reps) {
  ...
  results_y1_sample[r, 6:8] <- with(sample_nonprob, quantile(y1, p_quantiles))
  results_y1_sample[r, 9:11] <- with(sample_nonprob, weightedQuantile(y1, w_cal_s, p_quantiles))
  results_y1_sample[r, 12:14] <- with(sample_nonprob, weightedQuantile(y1, w_cal_q1, p_quantiles))
  results_y1_sample[r, 15:17] <- with(sample_nonprob, weightedQuantile(y1, w_cal_q2, p_quantiles))
  ...
  results_y2_sample[r, 6:8] <- with(sample_nonprob, quantile(y2, p_quantiles))
  results_y2_sample[r, 9:11] <- with(sample_nonprob, weightedQuantile(y2, w_cal_s, p_quantiles))
  results_y2_sample[r, 12:14] <- with(sample_nonprob, weightedQuantile(y2, w_cal_q1, p_quantiles))
  results_y2_sample[r, 15:17] <- with(sample_nonprob, weightedQuantile(y2, w_cal_q2, p_quantiles))
  ...
  results_y3_sample[r, 6:8] <- with(sample_nonprob, quantile(y3, p_quantiles))
  results_y3_sample[r, 9:11] <- with(sample_nonprob, weightedQuantile(y3, w_cal_s, p_quantiles))
  results_y3_sample[r, 12:14] <- with(sample_nonprob, weightedQuantile(y3, w_cal_q1, p_quantiles))
  results_y3_sample[r, 15:17] <- with(sample_nonprob, weightedQuantile(y3, w_cal_q2, p_quantiles))
...
setnames(results_sample, names(results_sample), 
         c("iter", "m_naive", "m_cal", "m_qcal1", "m_qcal2",
           paste(paste0("q", rep(c(25, 50, 75), times = 4)), 
                 rep(c("naive", "cal", "qcal1", "qcal2"), each = 3), sep="_")))
...
results_sample[, type:= factor(type, c("naive", "cal", "qcal1", "qcal2"))]
...
p2
ggsave(plot = p2, filename = "../figs/sim-nonprob-estimated.pdf", width = 8, height = 6)
```
